...
into the RDS Postgres schema (schema.sql).

Connection settings are read from environment variables — nothing hardcoded:
    export PGHOST=<your-rds-endpoint>
    export PGDATABASE=vahan
    export PGUSER=postgres
    export PGPASSWORD=<your-master-password>

Run on EC2 (same instance as everything else):
    pip3.11 install pandas boto3 sqlalchemy psycopg2-binary --break-system-packages
...
def load_table(df: pd.DataFrame, table: str, engine, if_exists="append") -> None:
    df.to_sql(table, engine, if_exists=if_exists, index=False, method="multi", chunksize=5000)
